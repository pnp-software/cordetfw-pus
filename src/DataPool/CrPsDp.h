...
enum {
   ...
   DpIdfailCodeAccFailed = 65,
   DpIdfailCodePrgrFailed = 66,
   DpIdfailCodeStartFailed = 67,
   DpIdfailCodeTermFailed = 68,
   DpIdinvDestRerouting = 69,
   DpIdnOfAccFailed = 70,
   DpIdnOfPrgrFailed = 71,
   DpIdnOfReroutingFailed = 72,
   DpIdnOfStartFailed = 73,
   DpIdnOfTermFailed = 74,
   DpIdpcktIdAccFailed = 75,
   DpIdpcktIdPrgrFailed = 76,
   DpIdpcktIdReroutingFailed = 77,
   DpIdpcktIdStartFailed = 78,
   DpIdpcktIdTermFailed = 79,
   DpIdstepPrgrFailed = 80,
   DpIdverFailData = 81
};
 ...
 * to.\return Number of bytes copied. 0 if id is invalid.
 */
extern int setDpValueEx(ParameterId_t id, const void* src, void** dest, size_t* pElementLength, unsigned int* pNElements);
/**
