...
\SetDocIssue{0.3 -- DRAFT}
\SetDocRefNumber{PP-SP-PUX-0001}
\SetDocTitle{CORDET Framework - PUS Extension}
\SetDocSubtitle{Specification}
...
{
\pnpcsvtable[filter equal={\Category}{#1}]{|l|p{11.8cm}|}{#2}{tab:Req-#1}{\textbf{Req. ID} & \textbf{Requirement Text}}{GeneratedTables/PusExtensionReq.csv}{P-\Category-\Id/\Type & \textit{\Text}}
}
...
{
\pnpcsvtable[filter equal={\Category}{#1}]{|l|p{4.7cm}|p{6.9cm}|}{#2}{tab:AP-#1}{\textbf{AP ID} & \textbf{Adaptation Point} & \textbf{Close-Out Value}}{PusExtensionAP.csv}{P-\Category-\Id & \AP (\Origin) & \DefValue}
}
...
\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.35,keepaspectratio=true]{HierarchicalDefServ.png}
...
\hline
11 & Time-Based Scheduling Service & \ref{sec:serv11} \\
\hline
...
\item \textit{Multiplicity}: a positive integere which is either 1 for data items of primitive types or the number of elements for array-like data items
...
Array-like data pool items (i.e. data pool items with multiplicity greater than 1) may only contains elements of the same type. The type of the array-like data pool item is then the type of its elements. 

Array-like data pool items have two sets of identifiers: one identifier for the data pool item itself and then individual identifiers for each element in the data pool item. This allows user to address either the entire array as a single entity or each one of its elements. If N is the identifier of an array-like data pool items with m elements, then the identifiers of its elements are: (N+1), (N+2), ... (N+m).
...
\pnpfigure[scale=0.57]{The Data Pool State Machine}{fig:CrPsDataPool}{CrPsDataPool.png}
...
\pnpcsvtable[filter equal={\Domain}{Fct}]{|p{4cm}|>{\raggedright\arraybackslash}p{9cm}|}{Observables for Verification Service}{tab:Obs-FAC}{Name & Description}{./GeneratedTables/DataPool.csv}{\texttt{\Name} & \ShortDesc}
...
\begin{crReq}{S01}{Requirements for Service 1 (Request Verification)}
\end{crReq}
...
\begin{figure}[H]
 \centering
 \includegraphics[scale=0.6,keepaspectratio=true]{CrPsPcktReroutingFail.png}
 \caption{Packet Rerouting Failure Procedure}
 \label{fig:CrPsPcktReroutingFail}
\end{figure}

\begin{figure}[htbp]
 \centering
 \includegraphics[scale=0.6,keepaspectratio=true]{CrPsCmdVerSucc.png}
 \caption{Command Verification Success Procedure}
 \label{fig:CrPsCmdVerSucc}
\end{figure}

\begin{figure}[htbp]
 \centering
 \includegraphics[scale=0.6,keepaspectratio=true]{CrPsCmdVerFail.png}
 \caption{Command Verification Failure Procedure}
 \label{fig:CrPsCmdVerFail}
\end{figure}

\begin{figure}[htbp]
 \centering
 \includegraphics[scale=0.6,keepaspectratio=true]{CrPsCmdPrgrSucc.png}
 \caption{Command Progress Success Procedure}
 \label{fig:CrPsCmdPrgrSucc}
\end{figure}

\begin{figure}[H]
 \centering
 \includegraphics[scale=0.6,keepaspectratio=true]{CrPsCmdPrgrFail.png}
 \caption{Command Progress Failure Procedure}
 \label{fig:CrPsCmdPrgrFail}
\end{figure}
...
\hline
2 & \texttt{period} & Collection period in units of HK\_COLLECT\_PER & Positive integer (periodic reports) or zero (one-shot reports) \\
\hline
3 & \texttt{cycleCnt} & Cycle counter (see definition of service 3 reports and commands) & Integer in the range: 0..(\texttt{period}-1) \\
\hline
4 & \texttt{isEnabled} & True if the report is enabled & None  \\
\hline
5 & \texttt{dest} & The identifier of the application to which the report is sent & None \\
\hline
6 & \texttt{nSimple} & Number of simply-commutated data items in the report & Integer in range: 1..HK\_MAX\_N\_SIMPLE \\
\hline
7 & \texttt{rep[]} & List of super commutated sample repetition numbers (\texttt{rep[1]} .. \texttt{rep[nGroup]}) & The number of groups is in the range: 0..HK\_MAX\_N\_GR and each repetition number is in the range: 1..HK\_MAX\_REP \\
\hline
8 & \texttt{nRep[]} & List of numbers (\texttt{nRep[1]} .. \texttt{nRep[nGroup]}) of data items in each super-commutated group & Each \texttt{nRep[i]} is in range: 1..HK\_MAX\_N\_REP \\
\hline
9 & \texttt{lstId} & List of identifiers of data items in the report & Not more than HK\_MAX\_N\_ITEMS data items and each identifier is in range: [\texttt{DpIdParamsLowest},\texttt{DpIdVarsHighest}]
 \\
\hline
10 & \texttt{sampleBufId} & The identifier of the sampling buffer holding the super-commutated data item values & An integer in the range: 1..HK\_N\_SAMP\_BUF \\
\hline
\end{pnptable}  
...
\newpage
\printOutCmpHkRepRepSpec{|c|p{10cm}|}
\newpage
\printOutCmpHkDiagRepSpec{|c|p{10cm}|}
\newpage
\printInCmdHkOneShotHkCmdSpec{|c|p{10cm}|}
\newpage
\printInCmdHkOneShotDiagCmdSpec{|c|p{10cm}|}
\newpage
\printInCmdHkModPerHkCmdSpec{|c|p{10cm}|}
\newpage
\printInCmdHkModPerDiagCmdSpec{|c|p{10cm}|}
...
\pnpcsvtable[filter equal={\Domain}{Hk}]{|p{3.5cm}|>{\raggedright\arraybackslash}p{9.5cm}|}{Constants for Housekeeping Service}{tab:Const-S3}{Name & Description}{./GeneratedTables/Constants.csv}{\texttt{\Name} & \Desc}
...
\pnpcsvtable[filter equal={\Domain}{Hk}]{|p{2.5cm}|l|>{\raggedright\arraybackslash}p{5.5cm}|c|}{Observables and Parameters for Housekeeping Service}{tab:Obs-S3}{Name & Kind & Description & Multiplicity}{./GeneratedTables/DataPool.csv}{\texttt{\Name} & \Kind & \ShortDesc & \Multiplicity}

%---------------------------------------------------------------------------------
\subsection{Service 3 Adaptation Points}\label{sec:serv3AP}
Table \ref{tab:AP-S3} lists the CORDET Framework adaptation points for the housekeeping service. 

\begin{crAp}{S3}{Adaptation Points for Service 3 (Housekeeping)}
\end{crAp}
...
\begin{crReq}{S03}{Requirements for Service 3 (Housekeeping Service)}
\end{crReq}
...
In order to support the generation of the event reports it pre-defines, the PUS Extension defines the \textit{Generate Pre-Defined Event Functions}. These functions are responsible for: retrieving the event report from the OutFactory; configuring it in accordance with its event identifier; and loading it in the OutLoader. If the attempt to retrieve the event report from the OutFactory fails, the function generates an error report of type OUTFACTORY\_FAIL.
...
\begin{itemize}
\item The update of observables which are related to the \textit{occurrence of an event} is done by the Enable Check of component EvtRep. This is appropriate because this action is executed every time an application creates an event report. The update of observables which are related to the \textit{generation of an event} is done by the Update Action of component EvtRep. This is appropriate because this action is only executed when an event report is actually issued by an application.
...
\item Command (5,7) triggers the generation of a (5,8) report carrying all disabled identifiers. The PUS standard does not specify what should be done if the disabled event identifiers do not fit into one report. In order to cover this contingency, the PUS Framework may generate multiple (5,8) reports in response to one (5,7) command.
\end{itemize}

\newpage
\printOutCmpEvtInfoSevRepSpec{|c|p{10cm}|}
\printOutCmpEvtLowSevRepSpec{|c|p{10cm}|}
\newpage
\printOutCmpEvtMedSevRepSpec{|c|p{10cm}|}
\printOutCmpEvtHighSevRepSpec{|c|p{10cm}|}
\newpage
...
%---------------------------------------------------------------------------------
\subsection{Service 5 Adaptation Points}\label{sec:serv5AP}
Table \ref{tab:AP-S5} lists the CORDET Framework adaptation points for the event reporting service. 

\begin{crAp}{S5}{Adaptation Points for Service 5 (Event Reporting)}
\end{crAp}
...
\begin{crReq}{S05}{Requirements for Service 5 (Event Reporting Service)}
\end{crReq}
...
\begin{itemize}
\item The checks on the legality of the time-scheduled command are done in the Start Action of the (11,4) command (see table \ref{tab:InCmdScdInsTbaCmdSpec} and in particular figure \ref{fig:Cmd11s4Start})
\item The insertion of the TBA in the TBS is done by the Progress Action of the (11,4) command (see table \ref{tab:InCmdScdInsTbaCmdSpec})
\item The extraction of the TBA from the TBS and the loading of its embedded command in an InManager is done by the Time-Based Schedule Execution Procedure of figure \ref{fig:CrPsTbsExec}
\end{itemize}
...
Tables \ref{tab:InCmdScdEnbTbsCmdSpec} to \ref{tab:OutCmpScdGrpRepSpec} formally specify the service 11 commands and reports by specifying how the actions, checks and attributes of generic out-going commands and reports are specialized for this service (see section \ref{sec:defPusRepCmd}). 
...
\pnpfigure[scale=0.5]{Time-Based Schedule Execution Procedure}{fig:CrPsTbsExec}{CrPsTbsExec.png}
...
Data pool items may have a multiplicity greater than 1 (array-like data items). In this case, monitoring may be done either on the individual elements of the data item array or on the data item array as a whole (it is recalled that the data pool concept allows both the array as a whole or its individual elements to be separately addressd, see section \ref{sec:dpConcepts}).

The behaviour of the parameter monitoring function is described by the Parameter Monitoring Procedure of figure \ref{fig:CrPsMonFncPr}. The procedure is started when the parameter monitoring function becomes enabled and it is stopped when it becomes disabled. Thus, the enable status of the parameter monitoring function is given by the status of the Parameter Monitoring Procedure.
...
\item If an event is associated to the parameter monitor (i.e. if field \texttt{evtId} in table \ref{tab:pmonAtt} is different from zero), then the Generate Predefined Event Function is called to generate the event
\item If the parameter monitor is part of one or more functional monitors (i.e. if there are non-zero entries in \texttt{fMonList} in table \ref{tab:pmonAtt}), then the functional monitors are notified
\end{itemize}

The ground can choose whether or not an event is associated to a monitor and it can choose the identifier of the event but the layout of the event must be like that of the EVT\_MON\_VIOL event.
...
\hline
\texttt{monPrPrevRetVal} & Previous return value of the Monitor Procedure (or MON\_UNCHECKED) after the monitoring procedure or the monitoring function has been enabled) & See table \ref{tab:retValMonPr} \\
\hline
...
\begin{itemize}
\item Limit Check Monitor Procedure: verifies whether the value of the monitored parameter is within a pre-defined interval. See figure \ref{fig:CrPsMonLimCheckPr}.
\item Expected Value Monitor Procedure: verifies whether the monitored parameter has a pre-defined value. See figure \ref{fig:CrPsMonExpValPr}.
...
The Limit Chech Monitor Procedure and the Delta Check Monitor Procedure are "split by type" and come in two versions: one is applicable to integer-typed data items and the other is applicable to real-type data items. 
...
\texttt{MON\_UNCHECKED} & Parameter has not yet been checked \\
\hline
...
\hline
\texttt{MON\_DEL\_ABOVE} & Parameter delta-value (difference between successive values) is above its upper limit \\
\hline
\texttt{MON\_DEL\_BELOW} & Parameter delta-value (difference between successive values) is below its lower limit \\
\hline
\end{pnptable}  
...
Entries are added to the CTL by the Parameter Monitoring Procedure (see figure \ref{fig:CrPsMonFncPr}) when it wishes to report a monitoring violation. For this purpose, the framework provides an operation to add an entry to the CTL. When the first entry is added to the CTL, variable \texttt{ctlTimeFirstEntry} is updated with the current time.
...
\hline
\texttt{prevCheckStatus} & Checking status in the cycle before the violation was detected & See table \ref{tab:retValMonPr}  \\
\hline
\end{pnptable}  
...
Tables \ref{tab:InCmdMonEnbParMonDefCmdSpec} to \ref{tab:OutCmpMonRepFuncMonStatRepSpec} formally specify the service 12 commands and reports by specifying how the actions, checks and attributes of generic out-going commands and reports are specialized for this service (see section \ref{sec:defPusRepCmd}). 
...
\pnpcsvtable[filter equal={\Domain}{Mon}]{|p{3cm}|c|>{\raggedright\arraybackslash}p{9.5cm}|}{Observables and Parameters for On-Board Monitoring Service}{tab:Obs-S12}{Name & Kind & Description}{./GeneratedTables/DataPool.csv}{\texttt{\Name} & \Kind & \ShortDesc}
...
%-------------------------------------------------------------------------------------
\pnpfigure[scale=0.55]{Parameter Monitoring Procedure}{fig:CrPsMonFncPr}{CrPsMonFncPr.png}

\pnpfigure[scale=0.5]{Limit Check Monitor Procedure}{fig:CrPsMonLimCheckPr}{CrPsMonLimCheckPr.png}

\pnpfigure[scale=0.5]{Expected Value Monitor Procedure}{fig:CrPsMonExpValPr}{CrPsMonExpValPr.png}
...
Tables \ref{tab:OutCmpLptDownFirstRepSpec} to \ref{tab:InCmdLptAbortDownCmdSpec} formally specify the service 13 commands and reports by specifying how the actions, checks and attributes of generic out-going commands and reports are specialized for service 13 (see section \ref{sec:defPusRepCmd}). 
...
These components are defined by the way they close the adaptation points of the OutComponent and InCommand. This is defined formally in tables \ref{tab:InCmdTstAreYouAliveCmdSpec} to \ref{tab:OutCmpTstConnectRepSpec} but the main points are as follows.
...
\pnpcsvtable[filter equal={\Domain}{Tst}]{|p{3cm}|>{\raggedright\arraybackslash}p{9.5cm}|}{Constants for Test Service}{tab:Const-S17}{Name & Description}{./GeneratedTables/Constants.csv}{\texttt{\Name} & \Desc}
...
%=============================================================================================
\section{On-Board Parameter Management}\label{sec:serv20}
The service type of the Parameter Management Service is 20. The PUS Extension of the CORDET Framework supports this service in part.

The on-board parameter management service provides the capability to manage on-board parameters. In the PUS Extension, an "on-board parameter" is a data item in the data pool (see section \ref{sec:dp}). The definition of an "on-board parameter" is therefore as follows:

\begin{itemize}
\item The \textit{on-board parameter identifier} is the data pool identifier
\item The \textit{memory identifier} is the same for all on-board parameters (since on-board parameters reside in the data pool and an application can only have one data pool)
\item The \textit{address} of the on-board parameter identifier is the address of the data pool item
\item The \textit{packet field code} is derived from the type of the data pool item
\end{itemize}

It is recalled that the data pool concept of section \ref{sec:dpConcepts} provides  distinct identifiers for an array-like data pool item and for its elemens. Hence, the commands and reports in this service may be used to update or report the values both of an entire array or of one of its elements.

In reference [PS-SP], the On-Board Parameter Management Service provides the capability to report and update the values of on-board parameters and it provides the capability to dynamically modify the parameter definition. The PUS Extension only supports the first capability (i.e. it only supports sub-services 1, 2 and 3). Note that some of the functionality provided by the dynamic parameter re-definition is provided within the PUS Extension by the 'Debug Variable' concept of section \ref{sec:debugVar}.

The PUS Extension defines neither constants, nor variable, nor parameters, nor adaptation points for this service.

%-------------------------------------------------------------------------------------
\subsection{Service 20 Report and Command Definition}\label{sec:serv20RepCmdDef}
Tables \ref{tab:OutCmpLptDownFirstRepSpec} to \ref{tab:InCmdLptAbortDownCmdSpec} formally specify the service 13 commands and reports by specifying how the actions, checks and attributes of generic out-going commands and reports are specialized for service 13 (see section \ref{sec:defPusRepCmd}). 

\newpage
\printInCmdParRepParValCmdSpec{|c|p{10cm}|}

\newpage
\printOutCmpParParValRepSpec{|c|p{10cm}|}
\printInCmdParSetParValCmdSpec{|c|p{10cm}|}

%-------------------------------------------------------------------------------------
\newpage
\subsection{Service 20 Requirements}
The table in this section lists requirements for the test service.

\begin{crReq}{S20}{Requirements for Service 20 (On-Board Parameter Service)}
\end{crReq}
