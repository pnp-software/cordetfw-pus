...
\subsubsection{Reporting Successful Progress of Execution}
A Successful Progress of Execution Report (1,5) is generated when the progress action of an incoming command has completed one "progress step". In the CORDET Framework, a "progress step" corresponds to the execution of an InCommand in state PROGRESS. It is recognized that, in a PUS context, a "progress step" should have a higher granularity. For this reason, an adaptation point is defined (adaptation point ICM-15, "Operation to Report Progress Success for InCommand") which allows applications to specify the conditions under which a (1,5) report should be generated. 
...
\hline
Progress Step & In the PUS, the Progress Step is an enumerated type. In the CORDET Framework it is a positive integer which, by default, is equal to the number of times that the Progress Action has been executed since the execution of the command started. Applications can define a different granularity by overriding adaptation point ICM-15 (Operation to Report Progress Success for InCommand) \\
\hline
