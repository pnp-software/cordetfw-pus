...
#===============================================================================
# Write the getter and setter functions for a packet array parameter to a string 
# and return the string.
# Two getter functions and one setter function are generated:
# - Function to return a pointer to the array
# - Function to copy the content of the array to a caller-defined destination
# - Function to copy a content to the array parameter 
# .
# This version of the function does not handle the endianity of the parameters
# in the array (see GitHub ticket 4)
def writePcktArrayParFunctions(parName, pcktName, servName, parType):
    s = writeDoxy(['Getter function for pointer to parameter array '+parName+\
                   ' in packet '+pcktName,
                   '@param p Pointer to the packet',
                   '@return Pointer to the array  '+parName+' in packet '+pcktName])
    s = s + 'static inline ' + parType + '* get' + \
        servName + pcktName + parName + 'Array(void* p) {\n'
    s = s + '    ' + pcktName + '_t* t;\n'
    s = s + '    t = (' + pcktName + '_t*)p;\n'
    s = s + '    return &t->'+parName+'[0];\n}\n\n'
    
    s = s + writeDoxy(['Getter function for  content of array '+parName+' in packet '+pcktName,
                       '@param p Pointer to the packet',
                       '@param dest Pointer to memory location where array content is copied to'])
    s = s + 'static inline void get' + \
        servName + pcktName + parName + '(void* p, void* dest) {\n'
    s = s + '    ' + pcktName + '_t* t;\n'
    s = s + '    t = (' + pcktName + '_t*)p;\n'
    s = s + '    memcpy(dest, &t->'+parName+'[0], sizeof(t->'+parName+'));\n}\n\n'

    s = s + writeDoxy(['Setter function to load a content in array '+parName+' of packet '+pcktName,
                       '@param p Pointer to the packet',
                       '@param src Pointer to memory location where array content is copied from'])
    s = s + 'static inline void set' + \
        servName + pcktName + parName + '(void* p, void* src) {\n'
    s = s + '    ' + pcktName + '_t* t;\n'
    s = s + '    t = (' + pcktName + '_t*)p;\n'
    s = s + '    memcpy(&t->'+parName+'[0], src, sizeof(t->'+parName+'));\n}\n\n'
...
# If groupSizePar2 is equal to None, the parameter for which the getter method
# is generated is not part of a nested group; else, the parameter is part of a 
# nested group and groupSizePar2 is the parameter holding the size of the nested
# group.
def writePcktParGetterFunction(parName, pcktName, servName, parType, groupSizePar1, \
                               groupSizePar2=None):
